...
// Local change: the spring/easing constants this component shipped with are now
// the app's shared motion language (lib/motion.ts), so every other animated
// surface matches this one instead of re-deriving its own timings.
import { EASE_OUT, FADE, FADE_FAST, SPRING, SPRING_BOUNCY } from "@/lib/motion";
...
const HEADER_SPRING = SPRING_BOUNCY;
const ROW_SPRING = { ...SPRING, bounce: 0.26, delay: 0.08 } as const;
const ROW_OFFSET = 16;
const CELL_FADE = FADE;
const TOOLTIP_FADE = FADE_FAST;
...
export type GitHubActivityProps = React.ComponentProps<"div"> & {
  ...
  showMonths?: boolean;
  label?: string;
  /** Local addition: what the grid is counting, for the heading ("142 memories in 2026"). */
  noun?: string;
  defaultOpen?: boolean;
  open?: boolean;
...
const GitHubActivity = ({
  ...
  const heading = `${total} ${noun}${displayYear ? ` in ${displayYear}` : ""}`;
  ...
  return (
    <div
      ...
      className={cn(
        // Local change: was `bg-white dark:bg-black`, which ignored the app's
        // own surface tokens (and beat any bg-* class passed in, being a variant).
        "relative max-w-full overflow-hidden rounded-surface bg-background p-4",
