...
// Mirrors docs/AI_REQUIREMENTS.md's ingestion state machine, with four
...
// the user entered — DetectEvent right after ClassifyIntent, guessing
// whether this memory is date-bound (an event/deadline/appointment) so the
// app can proactively offer to add it to the user's calendar — and
// OrganizeCollection between GenerateAIInsights and SemanticChunker,
// deciding whether this memory belongs in an existing collection, warrants
// a new one, or neither.
// RouteMediaType -> (per-media-type parser) -> CorrectCaption ->
// DetectContentType -> ClassifyIntent -> DetectEvent -> GenerateAIInsights ->
...
const builder = new StateGraph(IngestionState)
  ...
  .addNode("detectEvent", detectEvent)
...
builder
  ...
  .addEdge("classifyIntent", "detectEvent")
  .addEdge("detectEvent", "generateAiInsights")
